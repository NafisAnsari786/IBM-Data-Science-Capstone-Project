...
import pandas as pd
import dash
from dash import html, dcc  # Update imports
...
app.layout = html.Div(children=[
    ...
    dcc.RangeSlider(
        id='payload-slider',
        min=int(min_payload),  # Convert min_payload to integer
        max=int(max_payload),  # Convert max_payload to integer
        step=1000,
        marks={i: str(i) for i in range(int(min_payload), int(max_payload) + 1, 1000)},  # Convert to integers
        value=[int(min_payload), int(max_payload)]  # Convert to integers
    ),
